...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['user_id']) && isset($_POST['new_role'])) {
    ...
    $updateQuery = "UPDATE registered_user SET role = ? WHERE username = ?";
...
// Fetch customer data
$sql = "SELECT ru.username, ru.email, ru.role, up.phone, up.address
        FROM registered_users ru
        LEFT JOIN user_profile up ON ru.username = up.username";
...
<section id="header">
    ...
    <div class="admin-content">
        <h2 style="text-align: center; color: #333; margin-bottom: 20px;">Customer Management</h2>
        <div>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= $row['role'] ?></td>
                            <td>
                                <form method="post" action="customer-management.php">
